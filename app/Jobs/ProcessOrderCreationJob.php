...
class ProcessOrderCreationJob implements ShouldQueue
{
        ...
        try {
            foreach ($institutions as $institution) {
                DB::transaction(function () use ($institution) {
                    $this->processInstitution($institution);
                });
            }
        } catch (\Throwable $e) {
    ...
    protected function processInstitution(Institution $institution): void
    {
        $students = Student::query()
                           ->with(['diet.menus.foods', 'mealPreferences'])
                           ->where('institution_id', $institution->id)
                           ->get();

        //Skip Institution entirely if no students
        if ($students->isEmpty()) {
            return;
        }

        //Create order in order table (by institution) - only if students exist
        ...
        //Now we know what students eat by meal type in the order, we have to count now all the foods by type for the order
